...
#    arguments.append('-DCMAKE_BUILD_TYPE=Release')
  ...
  if creation & 0x08:   # ==>Run
    ...
    if toolchain.startswith('msvc'):
      ...
      xcsoar_app = project_name + '.exe'  #  jetzt so...
    elif toolchain.startswith('mgw'):
      xcsoar_app = project_name + '-MinGW.exe'
      # xcsoar_app = 'XCSoarAug-MinGW.exe'
    ...
    arguments = [build_dir + '/' + xcsoar_app, '-600x480', '-profile=August.prf', '-datapath=D:/Data/XCSoarData']
