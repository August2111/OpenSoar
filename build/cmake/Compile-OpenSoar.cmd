...
::=======================
set APP_TOOLCHAIN=%~1
set APP_NAME=OpenSoar
set APP_NAME_LC=opensoar

:: set APP_NAME=XCSoar
:: set APP_NAME_LC=xcsoar

:: MinGW: mgw112, mgw122
:: MSVC : msvc2022, msvc2026
:: Clang: clang15 
rem if "%APP_TOOLCHAIN%" == "" set APP_TOOLCHAIN=msvc2022
if not defined APP_TOOLCHAIN set APP_TOOLCHAIN=msvc2026
if not defined APP_NAME      set APP_NAME=OpenSoar
if not defined APP_NAME_LC   set APP_NAME_LC=xcsoar
set COMPILE_PARTS=%~2
if not defined COMPILE_PARTS  set COMPILE_PARTS=15


echo %CD%
echo APP_TOOLCHAIN = %APP_TOOLCHAIN%
...
set CMD=python build/cmake/python/Start-CMake-%APP_NAME%.py  %APP_NAME_LC% %APP_TOOLCHAIN% %COMPILE_PARTS%
echo Command: %cmd%
echo .
timeout /t 10
%cmd%
