...
#ifdef HAVE_SKYSIGHT
  void SetBasePosition(Angle position, std::unique_ptr<MapOverlay> &&_overlay) noexcept;
  std::unique_ptr<MapOverlay> overlay[max_skysight_overlays];  // instead 9, see SkySight
#else
