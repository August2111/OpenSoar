...
TaskActionsPanel::TaskActionsPanel(TaskManagerDialog &_dialog,
    ...
    : RowFormWidget(_dialog.GetLook()), dialog(_dialog), parent(_parent),
      active_task(_active_task), task_modified(_task_modified) {}

void TaskActionsPanel::SaveTask() {
...
inline void TaskActionsPanel::OnBrowseClicked() { parent.SetCurrent(1); }

inline void TaskActionsPanel::OnNewTaskClicked() {
  if ((active_task->TaskSize() < 2) ||
      (ShowMessageBox(_("Create new task?"), _("Task New"),
                      MB_YESNO | MB_ICONQUESTION) == IDYES)) {
    active_task->Clear();
    active_task->SetFactory(
...
inline void TaskActionsPanel::OnDeclareClicked() {
  const auto errors = active_task->CheckTask();
  if (IsError(errors)) {
...
inline void TaskActionsPanel::OnDownloadClicked() noexcept try {
  ...
  auto pilot =
      CommonInterface::GetComputerSettings().weglide.pilot; // the preset value
...
void TaskActionsPanel::ReClick() noexcept { dialog.TaskViewClicked(); }

void TaskActionsPanel::Prepare([[maybe_unused]] ContainerWindow &parent,
  ...
  if (settings.weglide.pilot.id != 0)
    AddButton(_("Download WeGlide task"), [this]() { OnDownloadClicked(); });
  AddButton(_("Download User WeGlide Task"),
            [this]() { OnUserWeGlideClicked(); });
