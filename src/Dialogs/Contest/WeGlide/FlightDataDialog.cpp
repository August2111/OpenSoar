...
public:
  ListPickerWidget(unsigned _num_items, unsigned _initial_value,
      ...
      : num_items(_num_items), initial_value(_initial_value),
        row_height(_row_height), visible(false), item_renderer(_item_renderer),
        dialog(_dialog), caption(_caption), help_text(_help_text),
  ...
  bool CanActivateItem(unsigned index) const noexcept override { return true; }
