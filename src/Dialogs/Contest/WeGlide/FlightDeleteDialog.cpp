...
class DeleteDialog final : public WidgetDialog {

public:
  DeleteDialog(Auto style, UI::SingleWindow &parent, const DialogLook &look,
...
private:
  ...
  enum DataFieldIndex { SPACER1, USER_ID, USER_TOKEN, FLIGHT_ID, SPACER3 };
...
bool DeleteWidget::SetFocus() noexcept {
  bool data_ok;
...
void DeleteWidget::Prepare(ContainerWindow &parent,
  ...
  data_fields[2] = AddInteger(
      _("Flight"),
      _("Take this from your WeGlide Profile. Or set to 0 if not used."),
      _T("%u"), _T("%u"), 1, 9999999, 1, flight_id, this);
...
bool DeleteWidget::ShowDeleteDialog() {
  DeleteDialog dialog(WidgetDialog::Auto{}, UIGlobals::GetMainWindow(),
                     UIGlobals::GetDialogLook(), _("Delete Flight"), this);
  ...
  if (do_upload) {
    ...
    return DeleteIGCFile(GetUser(), GetFlight());
  } else {
    return false;
  }
}
...
int FlightDeleteDialog(const WeGlide::User &user,
                      const uint_least32_t flight_id) noexcept {
  DeleteWidget widget(UIGlobals::GetDialogLook(), user, flight_id);

  return widget.ShowDeleteDialog();
}
